...
/-- Square root (integer, floor) using Newton's method with bounded iterations -/
def sqrt (n : Nat) : Nat :=
  if n == 0 then 0
  else if n == 1 then 1
  else
    -- Use bounded iteration to ensure termination
    -- Newton's method converges in O(log log n) iterations, so n iterations is overkill but safe
    ...
    -- Start with n/2 as initial guess, use n as fuel (more than enough)
    loop (n / 2 + 1) n
...
def removeLiquidity (state : AMMState) (msg : MsgContext)
  ...
  -- Check: cannot remove all liquidity (must keep MINIMUM_LIQUIDITY)
  require (state.totalSupply - liquidity >= MINIMUM_LIQUIDITY) "AMM: cannot drain pool"
...
theorem empty_not_initialized : AMMState.empty.isInitialized = false := by
  ...
  simp only [Nat.lt_irrefl, Bool.and_self]
  decide
